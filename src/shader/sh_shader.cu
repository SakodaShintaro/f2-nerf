  ...
  const uint32_t num_elements, const uint32_t degree, float * data_in, float * data_out)
{
  ...
  auto write_sh = [&]() {
    ...
    data_out[12] =
      0.3731763325901154f * z * (5.0f * z2 - 3.0f);  // sqrt(7)*z*(5*z2 - 3)/(4*sqrt(pi))
    data_out[13] =
    ...
    data_out[59] =
      -0.15645893386229404f * x * (x2 - 3.0f * y2) *
      (13.0f * z2 * (11.0f * z2 - 3.0f) - 27.0f * z2 +
