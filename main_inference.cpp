...
int main(int argc, char * argv[])
{
  ...
  for (int32_t i = 0; i < dataset.n_images_; i++) {
    ...
    ofs << "original\t";
    ofs << initial_pose[0][0].item<float>() << "\t";
    ofs << initial_pose[0][1].item<float>() << "\t";
    ofs << initial_pose[0][2].item<float>() << "\t";
    ofs << score_before << std::endl;

    // Added noise
    for (int32_t d = 0; d < kDirNum; d++) {
      torch::Tensor curr_pose = initial_pose.clone();
      curr_pose[0][0] += noise * kDx[d];
      curr_pose[0][2] += noise * kDz[d];
      ...
      ofs << "noised_" << d << "\t";
      ofs << curr_pose[0][0].item<float>() << "\t";
      ofs << curr_pose[0][1].item<float>() << "\t";
      ofs << curr_pose[0][2].item<float>() << "\t";
      ofs << score_noised << std::endl;
      ...
      ofs << "optimized_" << d << "\t";
      ofs << optimized_pose[0][0].item<float>() << "\t";
      ofs << optimized_pose[0][1].item<float>() << "\t";
      ofs << optimized_pose[0][2].item<float>() << "\t";
      ofs << score_after << std::endl;
    }
  }
}