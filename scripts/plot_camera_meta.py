...
if __name__ == "__main__":
    ...
    default_pose_front_l = np.array([-0.25, 0, -1, 1])
    default_pose_front_r = np.array([+0.25, 0, -1, 1])
    ...
    for i, pose in enumerate(tqdm(poses)):
        ...
        curr_front_l = np.dot(mat, default_pose_front_l)
        curr_front_r = np.dot(mat, default_pose_front_r)
        plt.plot(rear_pos[:i + 1, 2], rear_pos[:i + 1, 0])
        plt.plot(curr_axle[2], curr_axle[0])
        plt.arrow(curr_axle[2],
                  curr_axle[0],
                  curr_front_l[2] - curr_axle[2],
                  curr_front_l[0] - curr_axle[0],
                  width=0.1,
                  color="red")
        plt.plot(rear_pos[:i + 1, 2], rear_pos[:i + 1, 0])
        plt.plot(curr_axle[2], curr_axle[0])
        plt.arrow(curr_axle[2],
                  curr_axle[0],
                  curr_front_r[2] - curr_axle[2],
                  curr_front_r[0] - curr_axle[0],
                  width=0.2,
                  color="red")
        plt.axis("equal")
